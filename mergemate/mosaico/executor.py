...
MergeMateAgentExecutor.execute runs a single non-streaming path: it FIRST installs a
...
the inbound text to a mergemate command, and completes the Task with the rendered
markdown.
...
class MergeMateAgentExecutor(AgentExecutor):
    """Turns a MOSAICO message/send into a mergemate run and returns a Task."""
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        raise NotImplementedError("cancel is not supported by the MergeMate solution agent")


async def health_check() -> str:
    """LLM-connectivity probe for /health. NO-RETRY: bypasses mergemate's retry-wrapped
