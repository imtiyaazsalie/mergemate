...
# Fully load mergemate's config first. mergemate.log imports config_loader, whose
# module-level load_file() lazily triggers Dynaconf's custom_merge_loader, which in turn
# imports mergemate.log. Importing config_loader as the first mergemate import here ensures
...
import mergemate.config_loader  # noqa: F401  (import-order load; do not remove)

from mergemate.log import LoggingFormat, get_logger, setup_logger
from mergemate.mosaico.card import build_agent_card
from mergemate.mosaico.env_bridge import apply_mosaico_env, langfuse_env_present
from mergemate.mosaico.executor import MergeMateAgentExecutor, health_check
...
def _configure_runtime() -> None:
    ...
    sink as an import-time side effect runs *before* mergemate's lazy Dynaconf config has
    finished loading, which trips a circular import (mergemate.log <-> custom_merge_loader)
...
def build_app():
    ...
    handler = DefaultRequestHandler(
        agent_executor=MergeMateAgentExecutor(),
...
def start() -> None:
    ...
    app = build_app()
    get_logger().info(f"Starting MergeMate MOSAICO solution agent on {host}:{port}")
