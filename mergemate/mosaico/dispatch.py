"""Text router: turn inbound MOSAICO text into a mergemate command and return the
rendered markdown.
...
from mergemate.mosaico.diff_provider import parse_unified_diff
...
def _empty_fallback(verb: str) -> str:
    return f"MergeMate {verb}: no output produced (e.g. no files/changes detected)."


def _error_fallback(verb: str) -> str:
    return f"MergeMate could not complete the {verb} (internal error; see agent logs)."
...
def _ask_needs_context_fallback() -> str:
    ...
    return "MergeMate requires a PR URL or a supplied diff."
...
async def _fetch_public_diff(pr_url: str) -> Optional[str]:
    ...
    headers = {"User-Agent": "mergemate-mosaico"}
    try:
...
def _pr_fetch_failed_fallback(pr_url: str) -> str:
    return (
        f"MergeMate could not fetch a public diff for {pr_url} "
...
async def _run_mergemate(target: str, verb: str) -> "RouteResult":
    """Run a review/improve/describe verb via MergeMateAgent, defensively.
    Force non-publishing output capture: tools store formatted output in
    config._raw['__output_capture__'] when publish_output is False."""
    from mergemate.agent.mergemate import MergeMateAgent
    from mergemate.core.config import AppConfig

    # Mutable capture container — tool writes formatted output here when publishing is disabled.
    output_capture: list[str] = [""]
    config = AppConfig.load_default().with_overrides(publish_output=False)
    # Some tools (PRCodeSuggestions, PRDescription) read publish_output from _raw["config"] via _cfg()
    config._raw.setdefault("config", {})["publish_output"] = False
    config._raw["__output_capture__"] = output_capture

    agent = MergeMateAgent(config=config)
    ok = await agent.handle(target, ["/" + verb])

    if ok is False:
        return RouteResult(_error_fallback(verb), ok=False)
    artifact = output_capture[0].strip()
...
async def _run_ask(target: str, question: str) -> "RouteResult":
    """Run the ask path via MergeMateAgent with output capture."""
    from mergemate.agent.mergemate import MergeMateAgent
    from mergemate.core.config import AppConfig
    ...
    agent = MergeMateAgent(config=config)
    try:
        ok = await agent.handle(target, ["ask", question])
    except Exception:
...
async def _run_on_diff(diff_body: str, verb: str, text: str, title: str, empty_ok: bool = True) -> "RouteResult":
    ...
    return await _run_mergemate("mosaico://supplied-diff", verb)


async def route_and_run_result(user_text: str) -> "RouteResult":
    """Route inbound text to a mergemate command and return a RouteResult. Never raises."""
    try:
        text = user_text or ""
