"""DiffInputProvider — a GitProvider that feeds a SUPPLIED unified diff to mergemate's
...
class _PullRequestMimic:
    """Mimics the PullRequest object the tools touch (.title, .diff_files, .url)."""

    def __init__(self, title: str, diff_files: List[FilePatchInfo], url: str = ""):
        self.title = title
...
def parse_unified_diff(diff_text: str) -> List[FilePatchInfo]:
    ...
    patch = that file's section verbatim (the @@ hunk body mergemate's hunk processing
...
class DiffInputProvider(GitProvider):
    ...
    def __init__(self, pr_url: Optional[str] = None):
        ...
        self.pr = _PullRequestMimic(self._title, self.diff_files, url=pr_url or "")
