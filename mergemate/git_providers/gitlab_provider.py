...
class GitLabProvider(GitProvider):
    def __init__(self, merge_request_url: Optional[str] = None, incremental: Optional[bool] = False):
        ...
        if auth_method not in ["oauth_token", "private_token"]:
            raise ValueError(
                f"Unsupported GITLAB.AUTH_TYPE: '{auth_method}'. " f"Must be 'oauth_token' or 'private_token'."
            )
    ...
    # Example: https://gitlab.com/mergemate/mergemate.git and branch: t1 -> prefix: "https://gitlab.com/mergemate/mergemate/-/blob/t1", suffix: "?ref_type=heads"
    ...
    def get_repo_settings(self):
        try:
            ...
            contents = (
                self.gl.projects.get(self.id_project).files.get(file_path=".mergemate.toml", ref=main_branch).decode()
            )
            return contents
        except Exception:
            return ""
    ...
    def _prepare_clone_url_with_token(self, repo_url_to_clone: str) -> str | None:
        ...
        # For example: For repo url: https://gitlab.mergemate-inc.com/mergemate/autoscraper.git
        # Then to clone one will issue: 'git clone https://oauth2:<access token>@gitlab.mergemate-inc.com/mergemate/autoscraper.git'
