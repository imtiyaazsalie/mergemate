...
from mergemate.custom_merge_loader import validate_file_security
...
def _resolve_extra_config_to_file(source):
    ...
    Accepts:
      - http:// or https:// URL: fetched via urllib (with optional auth header
        from MERGEMATE_EXTRA_CONFIG_AUTH_HEADER, e.g. "PRIVATE-TOKEN: <token>").
    ...
    auth_header = os.environ.get("MERGEMATE_EXTRA_CONFIG_AUTH_HEADER")
    if auth_header:
        if ":" in auth_header:
...
def _reapply_env_overrides():
    ...
    config file or repo .mergemate.toml can silently replace a secret supplied
...
def _apply_settings_from_file(path: str, label: str):
    ...
    try:
        ...
        # Do NOT log the merged dict: external/repo .mergemate.toml may contain
...
def apply_repo_settings(pr_url):
    ...
    # extra file. Repo-local .mergemate.toml is still applied later and overrides
