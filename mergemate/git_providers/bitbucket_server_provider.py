...
class BitbucketServerProvider(GitProvider):
    def __init__(
        ...
        if bitbucket_client:  # if Bitbucket client is provided, use it
            ...
            if self.bearer_token:  # if bearer token is provided, use it
                self.bitbucket_client = Bitbucket(url=self.bitbucket_server_url, token=self.bearer_token)
            else:  # otherwise use username and password
                self.bitbucket_client = Bitbucket(url=self.bitbucket_server_url, username=username, password=password)
        try:
    ...
    def get_repo_settings(self):
        try:
            content = self.bitbucket_client.get_content_of_file(self.workspace_slug, self.repo_slug, ".mergemate.toml")
            ...
            get_logger().error(f"Failed to load .mergemate.toml file, error: {e}")
            return ""
