...
class GithubProvider(GitProvider):
    ...
    # Example: https://github.com/the-mergemate/mergemate.git and branch: v0.8 -> prefix: "https://github.com/the-mergemate/mergemate/blob/v0.8", suffix: ""
    ...
    def publish_code_suggestions(self, code_suggestions: list) -> bool:
        ...
        get_logger().debug(
            f"Inline suggestions: {len(code_suggestions)} in, {len(code_suggestions_validated)} validated"
        )
    ...
    def get_repo_settings(self):
        ...
        # settings value doesn't short-circuit the MERGEMATE_CONFIG_BRANCH fallback.
        ...
        env_branch = (os.environ.get("MERGEMATE_CONFIG_BRANCH") or "").strip()
        ...
        if config_branch:
            ...
            try:
                return self.repo_obj.get_contents(".mergemate.toml", ref=config_branch).decoded_content
            except GithubException as e:
                get_logger().warning(
                    f"Failed to load .mergemate.toml from branch '{config_branch}', falling back to default branch",
        ...
        if self.deployment_type == "app":
            ...
            try:
                ...
                raise ValueError(
                    ...
                    "https://github.com/MergeMate-ai/mergemate#method-2-run-from-source"
                ) from e
    ...
    def get_pr(self):
        """Public API for getting the PR as a MergeMate PullRequest."""
        ...
        return PullRequest(
            ...
            branch=gh_pr.head.ref if gh_pr.head else "",
            base_branch=gh_pr.base.ref if gh_pr.base else "",
            owner=self.repo.split("/")[0] if self.repo else "",
            repo=self.repo.split("/")[1] if self.repo and "/" in self.repo else "",
            number=self.pr_num or 0,
        )
