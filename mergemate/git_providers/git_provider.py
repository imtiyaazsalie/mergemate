...
from mergemate.algo.utils import Range, process_description
...
def get_git_ssl_env() -> dict[str, str]:
    ...
    if ssl_cert_file:
        if os.path.exists(ssl_cert_file):
            if (requests_ca_bundle and requests_ca_bundle != ssl_cert_file) or (
                git_ssl_ca_info and git_ssl_ca_info != ssl_cert_file
            ):
                get_logger().warning(
                    f"Found mismatch among: SSL_CERT_FILE, REQUESTS_CA_BUNDLE, GIT_SSL_CAINFO. "
                ...
                get_logger().info(
                    f"Using SSL certificate bundle for git operations", artifact={"ssl_cert_file": ssl_cert_file}
                )
            chosen_cert_file = ssl_cert_file
        else:
            get_logger().warning(
                "SSL certificate bundle not found for git operations", artifact={"ssl_cert_file": ssl_cert_file}
            )
    ...
    # Fallback to REQUESTS_CA_BUNDLE
        ...
        if os.path.exists(requests_ca_bundle):
            if git_ssl_ca_info and git_ssl_ca_info != requests_ca_bundle:
                get_logger().warning(
                    f"Found mismatch between: REQUESTS_CA_BUNDLE, GIT_SSL_CAINFO. "
            ...
            get_logger().warning(
                "requests CA bundle not found for git operations", artifact={"requests_ca_bundle": requests_ca_bundle}
            )
    ...
    # Fallback to GIT CA:
        ...
        if os.path.exists(git_ssl_ca_info):
            get_logger().info(
                "Using git SSL CA info from GIT_SSL_CAINFO for git operations",
                artifact={"git_ssl_ca_info": git_ssl_ca_info},
            )
            chosen_cert_file = git_ssl_ca_info
        else:
            get_logger().warning(
                "git SSL CA info not found for git operations", artifact={"git_ssl_ca_info": git_ssl_ca_info}
            )

    else:
        get_logger().warning(
            "Neither SSL_CERT_FILE nor REQUESTS_CA_BUNDLE nor GIT_SSL_CAINFO are defined, or they are defined but not found. Returning environment without SSL configuration"
        )
...
class GitProvider(ABC):
    ...
    def get_user_description(self) -> str:
        ...
        if user_description_header not in description_lowercase:
            get_logger().info(
                f"Existing description was generated by the mergemate, but it doesn't contain a user description"
            )
            return ""
...
def get_main_pr_language(languages, files) -> str:
    ...
    try:
        # Filter out non-numeric values (e.g., 'url' from GitHub API)
        lang_counts = {k: v for k, v in languages.items() if isinstance(v, (int, float))}
        top_language = max(lang_counts, key=lang_counts.get).lower()
