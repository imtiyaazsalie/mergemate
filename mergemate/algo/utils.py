...
def get_max_tokens(model):
    ...
    logic:
    (1) If the model is in './mergemate/algo/__init__.py', use the value from there.
    ...
    if model in MAX_TOKENS:
        ...
        get_logger().error(
            f"Model {model} is not defined in MAX_TOKENS in ./mergemate/algo/__init__.py and no custom_model_max_tokens is set"
        )
        raise Exception(
            f"Ensure {model} is defined in MAX_TOKENS in ./mergemate/algo/__init__.py or set a positive value for it in config.custom_model_max_tokens"
        )
...
def show_relevant_configurations(relevant_section: str) -> str:
    ...
    markdown_text += "<br>These are the relevant [configurations](https://github.com/MergeMate-ai/mergemate/blob/main/mergemate/settings/configuration.toml) for this tool:\n\n"
...
def process_description(description_full: str) -> Tuple[str, List]:
    ...
    if PRDescriptionHeader.FILE_WALKTHROUGH.value in description_full:
        ...
        if len(description_split) < 2:
            get_logger().error(
                "Failed to split description into base and changes walkthrough",
...
def get_version() -> str:
    ...
    try:
        return version("mergemate")
    except PackageNotFoundError:
        get_logger().warning("Unable to find package named 'mergemate'")
        return "unknown"
