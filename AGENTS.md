...
- **Do** adjust configuration through `.mergemate.toml` or files under `mergemate/settings/` instead of hard-coding values.
...
MergeMate automates AI-assisted reviews for pull requests across multiple git providers.

- `mergemate/agent/` orchestrates commands (`review`, `describe`, `improve`, etc.) via `mergemate/agent/mergemate.py`.
- `mergemate/tools/` implements individual capabilities such as reviewers, code suggestions, docs updates, and label generation.
- `mergemate/git_providers/` and `mergemate/identity_providers/` handle integrations with GitHub, GitLab, Bitbucket, Azure DevOps, and secrets.
- `mergemate/settings/` stores Dynaconf defaults (prompts, configuration templates, ignore lists) respected at runtime; `.mergemate.toml` overrides repository-level behavior.
...
- `.github/workflows/` defines CI pipelines for unit tests, coverage, docs deployment, pre-commit, and MergeMate self-review.
...
- Execute the CLI locally once dependencies and API keys are available: `python -m mergemate.cli --pr_url <https://host/org/repo/pull/123> review`.
...
- Configuration files in `mergemate/settings/` are TOML; preserve formatting, section order, and comments when editing prompts or defaults.
...
- Prefer focused unit tests that isolate helpers in `mergemate/algo/`, `mergemate/tools/`, or provider adapters; use parameterized tests where existing files already do so.
...
- Treat prompt and configuration files as single sources of truth—update mirrors (`.mergemate.toml`, `mergemate/settings/*.toml`) together when behavior changes.
...
- Adjust runtime behavior by overriding keys in `.mergemate.toml` or by supplying repository-specific Dynaconf files; keep overrides minimal and documented inside the PR description.
