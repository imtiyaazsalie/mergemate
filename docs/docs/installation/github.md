In this page we will cover how to install and run MergeMate as a GitHub Action or GitHub App, and how to configure it for your needs.
...
jobs:
  mergemate_job:
    ...
    permissions:
      ...
      pull-requests: write
      contents: write
    name: Run settings on every pull request, respond to user comments
    steps:
...
4) You may configure MergeMate by adding environment variables under the env section corresponding to any configurable property in the [configuration](https://github.com/mergemate/mergemate/blob/main/mergemate/settings/configuration.toml) file. Some examples:
...
This section provides detailed, step-by-step examples for configuring MergeMate with different models and advanced options in GitHub Actions.
...
Instead of setting all options via environment variables, you can use a `.mergemate.toml` file in your repository root:

1. Create a `.mergemate.toml` file in your repository root:
...
- **Solution**: Check the model name format and ensure it matches the exact identifier. See the [Changing a model in MergeMate](../usage-guide/changing_a_model.md) guide for supported models and their correct identifiers.
...
- [Changing a model in MergeMate](../usage-guide/changing_a_model.md)
...
!!! tip ""
    ...
    For enhanced security, you can also specify the Docker image by its [digest](https://hub.docker.com/repository/docker/mergemate/mergemate/tags):
    ```yaml
    ...
        steps:
          - name: MergeMate action step
...
- Set deployment_type to 'app' in [configuration.toml](https://github.com/mergemate/mergemate/blob/main/mergemate/settings/configuration.toml)
...
6) Build a Docker image for the app and optionally push it to a Docker repository. We'll use Dockerhub as an example:
    ...
    docker push mergemate/mergemate:github_app  # Push to your Docker repository
    ```
...
2. Build a docker image that can be used as a lambda function
    ...
    ```shell
    docker buildx build --platform=linux/amd64 . -t mergemate/mergemate:github_lambda --target github_lambda -f docker/Dockerfile.lambda
   ```
...
3. Push image to ECR
    ...
    ```shell
    docker tag mergemate/mergemate:github_lambda <AWS_ACCOUNT>.dkr.ecr.<AWS_REGION>.amazonaws.com/mergemate/mergemate:github_lambda
