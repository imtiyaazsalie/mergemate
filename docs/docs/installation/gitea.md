...
6. Build a Docker image for the app and optionally push it to a Docker repository. We'll use Dockerhub as an example:
    ...
    docker push mergemate/mergemate:gitea_webhook  # Push to your Docker repository
    ```
...
8. Create a webhook in your Gitea project. Set the URL to `http[s]://<MERGEMATE_HOSTNAME>/api/v1/gitea_webhooks`, the secret token to the generated secret from step 3, and enable the triggers `push`, `comments` and `merge request events`.
