...
(REAL RawContextMiddleware mounted), proving the full path:
  HTTP POST / -> RawContextMiddleware -> DefaultRequestHandler -> MergeMateAgentExecutor.execute
...
from mergemate.mosaico.server import build_app

pytestmark = pytest.mark.skip(reason="Full-path smoke integration — needs server stack")
