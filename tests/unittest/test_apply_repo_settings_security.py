"""
Security-focused tests for mergemate.git_providers.utils.apply_repo_settings.
...
@pytest.mark.xfail(
    reason=(
        "Behavior gap: mergemate.custom_merge_loader is invoked with silent=True, "
...
def test_temp_file_is_removed_after_failed_apply(monkeypatch, tmp_path, settings_snapshot):
    ...
    the ``Dynaconf`` symbol bound inside ``mergemate.git_providers.utils``
