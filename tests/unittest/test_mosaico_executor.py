"""Tests for MergeMateAgentExecutor.execute (A2A 1.0).
...
import mergemate.mosaico.executor as executor_mod
from mergemate.mosaico.dispatch import RouteResult
from mergemate.mosaico.executor import MergeMateAgentExecutor
...
class TestExecute:
    ...
    async def test_completes_with_artifact(self, monkeypatch, spy_updater):
        ...
        with request_cycle_context({}):
            await MergeMateAgentExecutor().execute(ctx, eq)
    ...
    async def test_empty_render_artifact_has_placeholder(self, monkeypatch, spy_updater):
        ...
        monkeypatch.setattr(executor_mod, "route_and_run_result", fake_route_and_run_result)
        with request_cycle_context({}):
            await MergeMateAgentExecutor().execute(_FakeRequestContext("x"), _RecordingEventQueue())
    ...
    async def test_route_and_run_raises_marks_failed(self, monkeypatch, spy_updater):
        ...
        eq = _RecordingEventQueue()
        with request_cycle_context({}):
            await MergeMateAgentExecutor().execute(_FakeRequestContext("y"), eq)
    ...
    async def test_failed_result_marks_failed_not_completed(self, monkeypatch, spy_updater):
        ...
        with request_cycle_context({}):
            await MergeMateAgentExecutor().execute(_FakeRequestContext("z"), _RecordingEventQueue())
    ...
    async def test_missing_a2a_fields_raise_controlled_error(self, monkeypatch, spy_updater, missing):
        ...
        with request_cycle_context({}):
            with pytest.raises(ValueError):
                await MergeMateAgentExecutor().execute(ctx, _RecordingEventQueue())
    ...
    async def test_cancel_raises_not_implemented(self):
        with pytest.raises(NotImplementedError):
            await MergeMateAgentExecutor().cancel(_FakeRequestContext("z"), _RecordingEventQueue())
