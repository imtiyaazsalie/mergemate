...
def _make_tool(git_provider=None):
    ...
    tool._cfg_config = {}
    tool.config = MagicMock(verbosity_level=0)
    return tool
...
async def test_push_inline_renders_body_with_score_and_label():
    ...
    await tool._push_inline_code_suggestions(data)
...
async def test_push_inline_publishes_no_suggestions_comment_when_empty():
    ...
    await tool._push_inline_code_suggestions({"code_suggestions": []})

    git_provider.publish_comment.assert_called_once_with("No suggestions found to improve this PR.")
...
def test_generate_summarized_suggestions_empty_returns_placeholder():
    tool = _make_tool()
    out = tool._generate_summarized_suggestions({"code_suggestions": []})
...
def test_generate_summarized_suggestions_renders_table_and_sorts_by_score():
    ...
    tool = _make_tool(git_provider)
    tool._cfg_suggestions["new_score_mechanism"] = False
    ...
    out = tool._generate_summarized_suggestions({"code_suggestions": [low, high]})
...
def test_generate_summarized_suggestions_uses_score_string_when_new_mechanism_enabled():
    ...
    tool = _make_tool(git_provider)
    tool._cfg_suggestions["new_score_mechanism"] = True
    out = tool._generate_summarized_suggestions(
...
def test_generate_summarized_suggestions_escapes_angle_bracket_strings_in_summary():
    ...
    out = tool._generate_summarized_suggestions({"code_suggestions": [suggestion]})
...
def test_validate_one_liner_zeroes_score_when_change_already_applied():
    ...
    out = tool._validate_one_liner_suggestion_not_repeating_code(suggestion)
...
def test_get_score_str_returns_bucket_for_default_thresholds(score, expected):
    ...
    assert tool._get_score_str(score) == expected
