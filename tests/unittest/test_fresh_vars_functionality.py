...
such as those introduced in https://github.com/the-settings/settings/pull/2087.
"""
...
# This ensures mergemate.config_loader is fully loaded before custom_merge_loader is used in tests
from mergemate.config_loader import get_settings  # noqa: F401
...
def create_dynaconf_with_custom_loader(temp_dir, secrets_file):
    ...
    which is the only way to configure it in settings.
    ...
    return Dynaconf(
        core_loaders=[],
        loaders=["mergemate.custom_merge_loader", "dynaconf.loaders.env_loader"],
        root_path=temp_dir,
        merge_enabled=True,
        envvar_prefix=False,
        load_dotenv=False,
...
"""
    ...
    def test_gitlab_personal_access_token_reload(self):
        ...
        # Set FRESH_VARS_FOR_DYNACONF environment variable (the only way to configure fresh_vars in settings)
