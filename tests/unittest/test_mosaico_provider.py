...
from mergemate.mosaico.diff_provider import DiffInputProvider, parse_unified_diff
...
async def _run_tool_via_spy(monkeypatch, isolated_settings, verb, canned_yaml, second_yaml=""):
    ...
    import mergemate.algo.ai_handlers.litellm_ai_handler as litellm_mod
    from mergemate.git_providers import _GIT_PROVIDERS
    ...
    config.publish_output = False  # Must be set on the config, not global_settings
    # PRCodeSuggestions/PRDescription use config._raw["config"]["publish_output"] via _cfg()
    ...
    ok = await agent.handle("https://github.com/test-org/test-repo/pull/1", ["/" + verb])
...
class TestSpikeEndToEnd:
    ...
    async def test_improve_end_to_end(self, monkeypatch, isolated_settings):
        ok, artifact, incremental = await _run_tool_via_spy(
            monkeypatch, isolated_settings, "improve", CANNED_IMPROVE_YAML, second_yaml=CANNED_IMPROVE_REFLECTION_YAML
        )
