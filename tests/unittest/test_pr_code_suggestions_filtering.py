...
def _make_tool(git_provider=None):
    ...
    tool._progress_response = None
    tool._cfg_suggestions = {}
    tool._cfg_config = {}
    tool.config = MagicMock(publish_output=False, verbosity_level=0)
    return tool
...
# ---------------------------------------------------------------------------
# _validate_one_liner_suggestion_not_repeating_code (stale-suggestion guard)
# ---------------------------------------------------------------------------
...
def test_validate_one_liner_marks_stale_suggestion_as_score_zero():
    ...
    git_provider.get_diff_files.return_value = _patch_files(base_file="return old()\n", head_file="return new()\n")
    ...
    result = tool._validate_one_liner_suggestion_not_repeating_code(suggestion)
...
def test_remove_line_numbers_strips_leading_digits_and_separator():
    tool = _make_tool()
    patch_input = ["## File: app.py\n1 def f():\n2     return old()\n10    return other()\n"]

    result = tool._remove_line_numbers(patch_input)
...
def test_remove_line_numbers_returns_original_on_exception():
    tool = _make_tool()
    # Exercise the broad ``except`` fallback by passing None in the list,
    # which has no ``splitlines``, triggering an exception.
    original_input = [None]

    result = tool._remove_line_numbers(original_input)

    assert result is original_input


# ---------------------------------------------------------------------------
# _analyze_self_reflection_response (reflection-mismatch / invalid output)
# ---------------------------------------------------------------------------
...
"""
    ...
    tool._analyze_self_reflection_response(data, response_reflect)

    for suggestion in data["code_suggestions"]:
        assert suggestion["score"] == 7
        assert suggestion["score_why"] == ""
...
async def test_analyze_self_reflection_clears_existing_code_when_equal_to_improved():
    ...
    git_provider.get_diff_files.return_value = []
    tool = _make_tool(git_provider)
    tool._cfg_suggestions["commitable_code_suggestions"] = False
...
async def test_analyze_self_reflection_clears_improved_code_in_commitable_mode():
    ...
    git_provider.get_diff_files.return_value = []
    tool = _make_tool(git_provider)
    tool._cfg_suggestions["commitable_code_suggestions"] = True
    data = {"code_suggestions": [_valid_suggestion(existing_code="same()", improved_code="same()")]}
    response_reflect = """
code_suggestions:
  - suggestion_score: 6
...
# ---------------------------------------------------------------------------
# _parse_code_suggestions filtering
# ---------------------------------------------------------------------------
...
"""
    ...
    data = tool._parse_code_suggestions(prediction)
