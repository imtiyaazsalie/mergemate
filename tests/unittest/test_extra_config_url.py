...
from mergemate.git_providers import utils as git_utils
from mergemate.git_providers.utils import (
...
class _CapturingHandler(BaseHTTPRequestHandler):
    ...
    body = SAMPLE_TOML
    expected_path = "/shared.mergemate.toml"
...
def http_server():
    ...
    _CapturingHandler.expected_path = "/shared.mergemate.toml"
...
def test_resolve_fetches_http_url_into_tempfile(http_server):
    url = f"{http_server}/shared.mergemate.toml"
    path, is_temp = _resolve_extra_config_to_file(url)
    try:
...
def test_resolve_injects_auth_header_from_env(http_server, monkeypatch):
    ...
    monkeypatch.setenv("MERGEMATE_EXTRA_CONFIG_AUTH_HEADER", f"PRIVATE-TOKEN: {fake_token}")
...
def test_resolve_returns_none_when_auth_header_missing(http_server, monkeypatch):
    ...
    monkeypatch.delenv("MERGEMATE_EXTRA_CONFIG_AUTH_HEADER", raising=False)
...
def test_resolve_malformed_auth_header_warns_and_drops(http_server, monkeypatch):
    ...
    _CapturingHandler.require_header = None
    monkeypatch.setenv("MERGEMATE_EXTRA_CONFIG_AUTH_HEADER", "no-colon-here")
...
def test_cli_parser_config_defaults_to_none_when_omitted():
    parser = build_parser()
    args = parser.parse_args(["--pr_url=https://example.com/pr/1", "review"])
    assert args.config is None


def test_cli_parser_accepts_model_flag():
    parser = build_parser()
    args = parser.parse_args(
        [
            "--pr_url=https://example.com/pr/1",
...
def test_cli_parser_model_defaults_to_none_when_omitted():
    parser = build_parser()
    args = parser.parse_args(["--pr_url=https://example.com/pr/1", "review"])
    assert args.model is None


def test_cli_run_passes_expected_config_to_handle(monkeypatch, tmp_path):
    """Each run() call builds an AppConfig from the supplied Namespace and
    passes it to _handle without leaking state between invocations."""
    from argparse import Namespace

    import mergemate.cli as cli_mod

    # Create a real config directory so _load_config won't fail on file I/O.
    config_dir = tmp_path / "first_config"
    config_dir.mkdir()
    config_file = config_dir / "configuration.toml"
    config_file.write_text('[config]\nmodel = "claude-sonnet-4-6"\n')

    alt_dir = tmp_path / "second_config"
    alt_dir.mkdir()
    alt_file = alt_dir / "configuration.toml"
    alt_file.write_text('[config]\nmodel = "gpt-4o"\n')

    configs_seen = []

    async def _fake_handle(url, request, config):
        configs_seen.append(config)
        return 0

    monkeypatch.setattr(cli_mod, "_handle", _fake_handle)

    # First invocation: a custom --config path
    cli_mod.run(
        args=Namespace(
            pr_url="https://example.com/pr/1",
            issue_url=None,
    ...
    first_config = configs_seen[0]

    # Second invocation: no --config — must produce a different, default config
    cli_mod.run(
        args=Namespace(
            pr_url="https://example.com/pr/1",
            issue_url=None,
    ...
    second_config = configs_seen[1]

    # Each invocation gets its own independent AppConfig instance (no global
    # singleton state to leak).
    assert first_config is not second_config, "successive run() calls must produce independent AppConfig instances"
    assert first_config is not second_config, "successive run() calls must produce independent AppConfig instances"
...
# configuration shipped by settings.
...
def test_apply_settings_file_does_not_log_secret_values(tmp_path, settings_sandbox):
    ...
    from the merged config, otherwise secrets in external .mergemate.toml
    ...
    settings uses loguru; pytest's capsys/caplog don't capture it because the
...
def test_extra_applied_when_repo_settings_empty(tmp_path, settings_sandbox, mock_git_provider):
    """If the repo has no .mergemate.toml, extra values still take effect."""
    extra_path = _write_toml(
        tmp_path,
        "extra.toml",
        f"""
...
def test_unreachable_extra_url_does_not_block_repo_settings(tmp_path, settings_sandbox, mock_git_provider):
...
""",
    ...
    assert get_settings().get(f"{_TEST_SECTION}.token") == "from-env", (
        ...
        "provider token from MERGEMATE_<SECTION>__<KEY> can be silently "
...
def test_env_var_overrides_repo_settings(tmp_path, settings_sandbox, mock_git_provider, monkeypatch):
    """Same precedence rule applies to the repo-local .mergemate.toml merge:
...
def test_env_var_visible_to_git_provider_after_extra_merge(tmp_path, settings_sandbox, monkeypatch):
    ...
    import mergemate.git_providers.utils as utils_mod
...
def test_resolve_accepts_windows_drive_letter_path(monkeypatch):
    ...
    monkeypatch.setattr(
        "mergemate.git_providers.utils.os.path.isfile",
...
def test_resolve_rejects_non_string_source():
    ...
    for bad in [42, ["https://x.example/y"], {"url": "x"}, True]:
        ...
        assert path is None and is_temp is False, f"non-string source {bad!r} must be rejected"
...
def test_safe_url_for_log_strips_credentials():
    ...
    from mergemate.git_providers.utils import _safe_url_for_log

    raw = "https://alice:s3cret@config.example.com:8443/settings/shared.toml?private_token=xyz"
    ...
    assert safe == "https://config.example.com:8443/settings/shared.toml"
...
def test_apply_settings_file_loads_when_dynaconf_lacks_security_flags(tmp_path, settings_sandbox, monkeypatch):
...
def test_apply_settings_file_security_check_runs_on_fallback_path(tmp_path, settings_sandbox, monkeypatch):
...
def test_env_var_overrides_repo_settings_when_default_exists(
    ...
    path: env vars must still win over repo-local .mergemate.toml for keys
