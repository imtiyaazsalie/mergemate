from mergemate.git_providers.utils import handle_configurations_errors
...
def test_handle_configurations_errors_uses_persistent_comment_when_supported():
    ...
    assert comment["initial_header"] == "❌ **MergeMate failed to apply 'local' repo settings**"
    assert comment["update_header"] is False
    assert comment["final_update_message"] is False
    assert "MergeMate failed to apply 'local' repo settings" in comment["body"]
