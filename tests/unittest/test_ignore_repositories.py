...
from mergemate.servers.bitbucket_app import should_process_pr_logic as bitbucket_should_process_pr_logic
from mergemate.servers.github_app import should_process_pr_logic as github_should_process_pr_logic
from mergemate.servers.gitlab_webhook import should_process_pr_logic as gitlab_should_process_pr_logic
    ...
    return {
        "data": {
            "pullrequest": {
                ...
                "destination": {"branch": {"name": "main"}, "repository": {"full_name": full_name}},
            },
...
def make_github_body(full_name):
    return {"pull_request": {}, "repository": {"full_name": full_name}, "sender": {"login": "user"}}


def make_gitlab_body(full_name):
    return {"object_attributes": {"title": "Test MR"}, "project": {"path_with_namespace": full_name}}
...
class TestIgnoreRepositories:
    ...
    def test_should_ignore_matching_repository(self, provider_name, provider_func, body_func):
        ...
        body = {"pull_request": {}, "repository": {"full_name": "org/repo-to-ignore"}, "sender": {"login": "user"}}
    ...
    def test_should_not_ignore_non_matching_repository(self, provider_name, provider_func, body_func):
        get_settings().set("CONFIG.IGNORE_REPOSITORIES", ["org/repo-to-ignore"])
        body = {"pull_request": {}, "repository": {"full_name": "org/other-repo"}, "sender": {"login": "user"}}
        result = provider_func(body_func(body["repository"]["full_name"]))
