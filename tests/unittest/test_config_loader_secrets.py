...
from mergemate.config_loader import apply_secrets_manager_config, apply_secrets_to_config


class TestConfigLoaderSecrets:
    def test_apply_secrets_manager_config_success(self):
        with (
            patch("mergemate.secret_providers.get_secret_provider") as mock_get_provider,
            patch("mergemate.config_loader.apply_secrets_to_config") as mock_apply_secrets,
            patch("mergemate.config_loader.get_settings") as mock_get_settings,
        ):
    ...
    def test_apply_secrets_manager_config_no_provider(self):
        with patch("mergemate.secret_providers.get_secret_provider") as mock_get_provider:
    ...
    def test_apply_secrets_manager_config_not_aws(self):
        with (
            ...
            assert not hasattr(mock_provider, "get_all_secrets") or not mock_provider.get_all_secrets.called

    def test_apply_secrets_to_config_nested_keys(self):
        with patch("mergemate.config_loader.get_settings") as mock_get_settings:
            settings = MagicMock()
            settings.get.return_value = None  # No existing value
            settings.set = MagicMock()
            mock_get_settings.return_value = settings

            secrets = {"openai.key": "sk-test", "github.webhook_secret": "webhook-secret"}
