"""Legacy litellm chat completion tests — skipped because the handler was rewritten.

The new LiteLLMAIHandler stores credentials per-instance (ProviderCredentials)
and passes them as kwargs to litellm.acompletion(). The old seed/temperature/
extended-thinking/_get_completion internal logic was intentionally removed.
"""
...
pytestmark = pytest.mark.skip(
    reason="Handler rewritten: seed/temperature/extended-thinking removed; uses ProviderCredentials"
)
...
async def test_chat_completion_passes_seed_when_temperature_is_zero(monkeypatch):
    ...
    with patch("mergemate.algo.ai_handlers.litellm_ai_handler.acompletion", new_callable=AsyncMock) as mock_call:
...
async def test_get_completion_uses_streaming_for_required_models():
    ...
    with (
        patch("mergemate.algo.ai_handlers.litellm_ai_handler.acompletion", new_callable=AsyncMock) as mock_call,
        patch(
            "mergemate.algo.ai_handlers.litellm_ai_handler._handle_streaming_response", new_callable=AsyncMock
        ) as mock_stream,
    ):
