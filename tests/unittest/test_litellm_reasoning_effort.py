...
def mock_logger():
    ...
    with patch("mergemate.algo.ai_handlers.litellm_ai_handler.get_logger") as mock_log:
...
@pytest.mark.skip(
    reason="Configurable reasoning_effort removed in handler rewrite; new handler always uses ReasoningEffort.HIGH for supported models"
)
class TestLiteLLMReasoningEffort:
    ...
    async def test_gpt5_valid_reasoning_effort_none(self, monkeypatch, mock_logger):
        ...
        with patch(
            "mergemate.algo.ai_handlers.litellm_ai_handler.acompletion", new_callable=AsyncMock
        ) as mock_completion:
