...
class TestPRUpdateChangelog:
    ...
    def mock_git_provider(self):
        ...
        provider.pr.branch = "feature-branch"
        provider.pr.description = "Test description"
        provider.pr.base_branch = "main"
    ...
    @pytest.fixture
    def mock_context(self, mock_git_provider):
        """Create a ToolContext with the PR data."""
        return ToolContext(
            pr=PullRequest(
    ...
    @pytest.fixture
    def changelog_tool(self, mock_git_provider, mock_ai_handler, mock_context, mock_config):
        """Create a PRUpdateChangelog instance with mocked dependencies."""
        with (
            patch("mergemate.tools.pr_update_changelog.get_main_pr_language", return_value="Python"),
            patch("mergemate.tools.pr_update_changelog.get_settings") as mock_settings,
        ):
            ...
            tool = PRUpdateChangelog(
                pr_url="https://gitlab.com/test/repo/-/merge_requests/1",
                config=mock_config,
    ...
    def test_get_changelog_file_with_exception(self, changelog_tool, mock_git_provider):
        ...
        # Assert
        assert changelog_tool.changelog_file == ""
        # In the rewritten code, exception path sets changelog_file_str to ""
        # before falling through to the if-not check which sets the default.
        # So changelog_file_str gets the default template.
        assert "Example:" in changelog_tool.changelog_file_str
    ...
    @pytest.mark.asyncio
    async def test_publish_with_push_support(self, changelog_tool, mock_git_provider):
        """Test _publish pushes changelog update when commit_changelog is enabled."""
        # Arrange
        mock_git_provider.create_or_update_pr_file = MagicMock()
        changelog_tool.commit_changelog = True
        changelog_tool.config.publish_output = True

        with (
            patch("mergemate.tools.pr_update_changelog.get_settings") as mock_settings,
            patch("mergemate.tools.pr_update_changelog.sleep"),
        ):
            ...
            result = {
                "new_file_content": "## v1.1.0\n- New feature",
                "answer": "## v1.1.0\n- New feature",
            }

            # Act
            await changelog_tool._publish(result)
    ...
    @pytest.mark.asyncio
    async def test_publish_without_commit_posts_comment(self, changelog_tool, mock_git_provider):
        """Test _publish posts a comment when commit_changelog is disabled."""
        # Arrange
        changelog_tool.commit_changelog = False
        changelog_tool.config.publish_output = True
        ...
        assert "Changelog updates" in mock_git_provider.publish_comment.call_args[0][0]
    ...
    @pytest.mark.parametrize(
        "existing_content,new_entry,expected_order",
        [
            (
                "# Changelog\n\n## v1.0.0\n- Old feature",
                "## v1.1.0\n- New feature",
