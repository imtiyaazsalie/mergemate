...
from mergemate.mosaico import dispatch
from mergemate.mosaico.dispatch import (
    RouteResult,
    _detect_verb,
    _empty_fallback,
    _error_fallback,
...
class TestPathPrUrl:
    ...
    async def test_pr_url_runs_handle_request_and_returns_artifact(self, monkeypatch, restore_settings):
        ...
        async def fake_handle(self, pr_url, request, *, notify=None):
            captured["pr_url"] = pr_url
            captured["request"] = request
            # Write to the output capture mechanism the dispatch uses
            cap = self.config._raw.get("__output_capture__")
            if cap is not None:
                cap[0] = "REVIEW MARKDOWN"
            return True
        ...
        monkeypatch.setattr(MergeMateAgent, "handle", fake_handle)
        ...
        # The verb is passed in the request list
...
class TestPathSuppliedDiff:
    ...
    async def test_diff_with_question_mark_in_body_still_reviews(self, monkeypatch, restore_settings):
        ...
        monkeypatch.setattr(MergeMateAgent, "handle", fake_handle)
        monkeypatch.setattr("mergemate.tools.pr_questions.PRQuestions", _explode_prquestions)
...
class TestPathFreeText:
    ...
    async def test_free_text_returns_guidance_not_internal_error(self, monkeypatch, restore_settings):
        """A context-free free-text ask must NOT call PRQuestions/MergeMateAgent and must NOT
        ...
        monkeypatch.setattr("mergemate.tools.pr_questions.PRQuestions", FakePRQuestions)

        out = await route_and_run("what does this codebase do?")
        assert out == "MergeMate requires a PR URL or a supplied diff."
...
class TestDefensiveCapture:
    ...
    async def test_ok_but_no_artifact_returns_empty_fallback(self, monkeypatch, restore_settings):
        ...
        async def fake_handle(self, pr_url, request, *, notify=None):
            # ok=True but never writes to __output_capture__ (early-return paths)
            return True
    ...
    async def test_ask_that_raises_returns_error_fallback(self, monkeypatch, restore_settings):
        ...
        monkeypatch.setattr(dispatch, "_fetch_public_diff", fake_fetch_public_diff)
        monkeypatch.setattr("mergemate.tools.pr_questions.PRQuestions", RaisingPRQuestions)
    ...
    async def test_route_and_run_never_raises_on_garbage(self, restore_settings):
        # No monkeypatching: a host-less MergeMate run / ask should still return a string.
...
class TestFetchPublicDiff:
    ...
    async def test_fetch_public_diff_non_200_returns_none(self, monkeypatch):
        session = _FakeSession({PR_URL + ".diff": _FakeResp(404, "not found")})
        monkeypatch.setattr(dispatch, "_url_is_safe", _always_safe)
        monkeypatch.setattr(aiohttp, "ClientSession", lambda *a, **k: session)
        assert await dispatch._fetch_public_diff(PR_URL) is None

    @pytest.mark.asyncio
    async def test_fetch_public_diff_oversize_returns_none(self, monkeypatch):
        big = "x" * (dispatch._DIFF_FETCH_MAX_BYTES + 1)
        session = _FakeSession({PR_URL + ".diff": _FakeResp(200, big)})
        monkeypatch.setattr(dispatch, "_url_is_safe", _always_safe)
        monkeypatch.setattr(aiohttp, "ClientSession", lambda *a, **k: session)
        assert await dispatch._fetch_public_diff(PR_URL) is None
    ...
    async def test_fetch_public_diff_assembles_multichunk_body(self, monkeypatch):
        session = _FakeSession({PR_URL + ".diff": _FakeResp(200, SAMPLE_RAW_DIFF)})
        monkeypatch.setattr(dispatch, "_url_is_safe", _always_safe)
        monkeypatch.setattr(aiohttp, "ClientSession", lambda *a, **k: session)
        body = await dispatch._fetch_public_diff(PR_URL)
        assert body == SAMPLE_RAW_DIFF


class TestFetchPublicDiffSSRF:
    @pytest.mark.asyncio
    async def test_non_https_blocked(self):
        assert await dispatch._url_is_safe("http://example.com") is False

    @pytest.mark.asyncio
    async def test_private_ip_blocked(self):
        assert await dispatch._url_is_safe("https://10.0.0.1/foo") is False
        assert await dispatch._url_is_safe("https://192.168.1.1/foo") is False

    @pytest.mark.asyncio
    async def test_metadata_ip_blocked(self):
        assert await dispatch._url_is_safe("https://169.254.169.254/foo") is False

    @pytest.mark.asyncio
    async def test_public_ip_allowed(self):
        assert await dispatch._url_is_safe("https://1.1.1.1/foo") is True
    ...
    async def test_redirect_to_internal_blocked(self, monkeypatch):
        ...
        monkeypatch.setattr("mergemate.mosaico.dispatch.socket.getaddrinfo", fake_getaddrinfo)

        class _RedirectSession:
            def get(self, url, allow_redirects=False):
                if url == PR_URL + ".diff":
                    return _FakeResp(302, "", {"Location": "https://evil.internal/diff"})
...
class TestPublishOutputForced:
    """Prove that _run_mergemate and _run_ask force publish_output=False regardless
    ...
    @pytest.mark.asyncio
    async def test_run_settings_injects_no_publish_flags(self, monkeypatch, restore_settings):
        """_run_mergemate must create a config with publish_output=False so that
        the tool writes into the output capture rather than publishing to the real PR."""
        captured_config = {}

        async def fake_handle(self, pr_url, request, *, notify=None):
            captured_config["publish_output"] = self.config.publish_output
            cap = self.config._raw.get("__output_capture__")
            if cap is not None:
                cap[0] = "REVIEW OUTPUT"
            return True
        ...
        from mergemate.agent.mergemate import MergeMateAgent
        from mergemate.mosaico.dispatch import _run_mergemate
        ...
        out = await _run_mergemate(PR_URL, "review")

        assert out.text == "REVIEW OUTPUT"
        # _run_mergemate sets publish_output=False on the config it creates
        assert captured_config.get("publish_output") is False, (
            "Production path must set publish_output=False on config; "
    ...
    async def test_run_ask_forces_publish_output_false(self, monkeypatch, restore_settings):
        """_run_ask must set publish_output=False before calling MergeMateAgent.handle()
        so that the publish guards in the tool never publish_comment to the real PR."""
        publish_output_at_run_time = {}

        async def fake_handle(self, pr_url, request, *, notify=None):
            publish_output_at_run_time["value"] = self.config.publish_output
            cap = self.config._raw.get("__output_capture__")
            if cap is not None:
                cap[0] = "CAPTURED ANSWER"
            return True
        ...
        from mergemate.mosaico.dispatch import _run_ask
        ...
        assert publish_output_at_run_time.get("value") is False, (
            "CONFIG.PUBLISH_OUTPUT must be False when agent.handle() is called; "
