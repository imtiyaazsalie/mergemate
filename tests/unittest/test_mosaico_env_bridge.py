"""Tests for settings/mosaico/env_bridge.py and observability.py.
...
from mergemate.mosaico.observability import mosaico_log_context, parse_observability_metadata
...
class TestApplyMosaicoEnv:
    ...
    def test_model_max_tokens_zero_falls_back_to_default(self, restore_settings, clear_mosaico_env, monkeypatch):
        ...
        from mergemate.mosaico.env_bridge import DEFAULT_CUSTOM_MODEL_MAX_TOKENS
    ...
    @pytest.mark.skip(
        reason="New LiteLLMAIHandler no longer mutates litellm module globals; uses instance-level credentials instead"
    )
