"""Unit tests for ``mergemate.servers.utils.DefaultDictWithTimeout`` and a few
helper functions in ``mergemate.servers.github_app``.
...
``mergemate.servers.utils`` module.
"""
...
from mergemate.servers import github_app
from mergemate.servers import utils as servers_utils
from mergemate.servers.utils import DefaultDictWithTimeout
...
def fake_clock(monkeypatch):
    """Replace ``time.monotonic`` in mergemate.servers.utils with a controllable clock."""
    state = {"t": 1_000_000.0}
